...
Goal: Introduce special relativity by focusing on why $c$, the speed of light, is fundamental and invariant.

Key Points: The postulate that $c$ is the same for all observers leads to time dilation and length contraction.
Explain how $c$ creates a relationship between space and time, where distance can be "subtracted" from time, giving rise to the spacetime interval.

Connection: This section sets up Minkowski space and real time, motivating the need for a geometric interpretation of spacetime.

## Lost In (Minkowski) Space
Goal: Introduce the geometry of Minkowski space and the concept of the spacetime interval.

Key Points: Define Minkowski space and the spacetime interval: $s^2 = c^2t^2 - x^2$. Explain that the interval $s$ remains invariant for all observers and represents real time (proper time), which serves as the "distance" in this geometry.

Connection: Introduce the idea that spacetime can be curved by mass, which lays the groundwork for general relativity and the Schwarzschild solution.
