...
- Schwartzchild's solution to General Relativity contained a curious mathematical singularity which---originally thought to be a _coordinate singularity_---turned out to be a real _curvature singularity_ in spacetime: the infamous **blackhole**.
...
# Absolute Power, Absolute Acceleration
...
2. Space and Time Adapt: For this invariance to hold, space and time themselves must adapt to keep the speed of light constant. If you move faster, time slows down and lengths contract to ensure you never catch up with light.
3. Relativity of Simultaneity: Events that appear simultaneous to one observer may not be simultaneous to another moving observer. This idea challenges our intuitive notion of a shared “now” and suggests that time flows differently depending on motion. Ab event in one observer's past can be another's future depending on how fast they are moving relative to one another.
4. More directly, we now know this also limits _informations_ ability to propagate faster than speed of light. Quantum entanglement, counter-intuitively, does _not_ violate this principle.
...
In everyday life, we think of **space** and **time** as separate. We measure distances in meters or miles and keep track of time with clocks. However, special relativity showed us that space and time are deeply connected, forming a unified framework called **spacetime**. We can described this framework mathematically using a special kind of geometry called **Minkowski space**.
...
In Minkowski space, the fundamental unit of "distance" isn’t a simple measurement of space or time alone—it’s a combination of both, called the **spacetime interval**. For two events separated by a distance $x$ and a time $t$, the spacetime interval $s$ is given by:
...
### Visualizing Minkowski Space

In Minkowski space, we can plot time and space as perpendicular axes, similar to a regular coordinate system, but with some key differences. Points (or “events”) in spacetime are located by both space and time coordinates. If we connect two events, the **spacetime interval** $s$ between them becomes the “distance” in this spacetime geometry.

This interval sets limits on causality: events separated by a spacetime interval greater than zero (i.e., spacelike intervals) cannot influence each other, as no signal can travel faster than the speed of light. Light itself defines the **boundary of possible influence**, forming a "light cone" structure in this space.

### Toward General Relativity: Curved Spacetime

Minkowski space describes a flat spacetime, where the geometry remains consistent everywhere. But what happens when we introduce **mass**? Einstein’s theory of general relativity builds on this framework by showing that **mass curves spacetime**. Massive objects like planets and stars create distortions, bending the paths of objects and even light. This curved geometry replaces Minkowski space with a more complex spacetime that adapts to the distribution of mass and energy.

This concept of curved spacetime is described mathematically by the **Schwarzschild solution**, a specific solution to Einstein’s field equations. It describes the geometry around a massive, non-rotating object, leading us to concepts like **black holes** and **event horizons**.

In summary, Minkowski space provides the foundation for understanding spacetime as a unified geometry where space and time are linked through the spacetime interval. This flat spacetime model prepares us for the next step: seeing how spacetime itself bends in the presence of mass, laying the groundwork for the profound ideas of general relativity.
...
Goal: Introduce general relativity’s core idea that mass and energy curve spacetime, with a specific focus on the Schwarzschild solution.

Key Points: Briefly explain how Einstein’s field equations describe how mass curves spacetime. Describe the Schwarzschild solution as a specific, spherically symmetric solution that describes the geometry outside a massive, non-rotating object. Explain the Schwarzschild radius and its role in defining the event horizon.

Connection: Use this to introduce black holes and explain why crossing the event horizon changes the nature of time and space.

## Black Holes: The Inevitable Exit from Our Universe
Goal: Define black holes, focusing on the event horizon and the singularity.

Key Points: Distinguish between the event horizon (boundary from which light cannot escape) and the singularity (where spacetime curvature becomes infinite). Describe how the event horizon represents a “point of no return”—once crossed, all paths lead to the singularity.

Connection: Lead into a discussion of Kruskal-Szekeres coordinates as a way to better understand spacetime inside the horizon.

## Kruskal-Szekeres Coordinates: Inside the Event Horizon

Goal: Use Kruskal coordinates to explain why the singularity lies in the future of any object crossing the event horizon.

Key Points:Explain how Kruskal coordinates extend the Schwarzschild solution, allowing us to visualize the paths of objects inside the event horizon. Describe how, inside the event horizon, the time and space axes effectively “flip”—$t$ becomes spatial, and $x$ becomes temporal. Discuss the inevitability of the singularity: once inside, all directions in time lead to the singularity.

Connection: This section ties together how both special and general relativity redefine our notions of space, time, and causality.
