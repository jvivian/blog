...
:::{.column width=70%}
```{python}
df[df.Year == 0][PROLOGUE].fillna(" ").style.hide(axis="index")
```
:::
...
::: {.column-page-inset}
```{python}
df[df.Year == 1][ND].fillna(' ').style.hide(axis='index')
```
:::
::: {.column-page}
![](imgs/year1.png)
...
:::{.column-page-inset}
```{python}
df[df.Year == 2][ND].fillna(' ').style.hide(axis='index')
```
:::{.column-screen}
...
:::{.column-page-inset}
```{python}
df[df.Year == 6][ND].fillna(' ').style.hide(axis='index')
```
:::
...
:::{.column-page-inset}
```{python}
df[df.Year == 7][COLS].fillna(' ').style.hide(axis='index')
```
:::
