...
subtitle: "Setting up Home Assistant for ZWave, MQTT, Discord, and Raspberry Pi integration"
author: "John Vivian"
date: "2023-03-02"
...
I had a Windows Surface laying around that I wasn't using and decided to turn it
into a server after a good friend got me excited about `hass`. As a masochistic exercise
I forewent reformatting with linux (in case Surface-specific issues arose)
and did everything in `WSL2` -- the Windows Subsystem for Linux.

Part 1 covers:

- Installing and running `hass` in `WSL2` on a Windows machine
- Setting up a **ZWave** USB Stick in WSL2
- Running the associated **ZWave** server in Docker
...
- Attach USB zwave stick to machine
...
- List devices to ensure zwave stick is present.
...
## Update Zwave server permissions

By default, the zwave server running in the docker does not support home assistant

- Go to `[localhost:8091](http://localhost:8091)`
- Settings → Home Assistant → WS Server

Home assistant should now be able to integrate ZWave automatically.
...
- Next, get the WSL2 IP (in WSL2 _not_ PowerShell)
...
Running `MQTT` is a breeze after setting up the `ZWave` server and handling the port-proxying for `hass`. [This guide](https://hometechhacker.com/mqtt-home-assistant-using-docker-eclipse-mosquitto/#What_is_MQTT_and_what_can_I_do_with_it) explains the set up in detail which I will list the core steps below:
