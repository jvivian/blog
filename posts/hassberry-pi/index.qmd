...
Running `MQTT` is a breeze after setting up the `Z-Wave` server and handling the port-proxying for `hass`. [This guide](https://hometechhacker.com/mqtt-home-assistant-using-docker-eclipse-mosquitto/#What_is_MQTT_and_what_can_I_do_with_it) explains the set up in detail which I will list the core steps below:
